...
// ══════════════════════════════════════
//  GROQ API — Routed through Netlify serverless proxy (key is safe server-side)
// ══════════════════════════════════════
// API key is stored in Netlify Environment Variables, NOT here.
// Go to: Netlify Dashboard → Site Settings → Environment Variables → Add GROQ_API_KEY
const GROQ_ENDPOINT = '/.netlify/functions/groq-proxy';

// Stubs kept for backward compatibility (no real keys here)
const GROQ_API_KEYS = ['proxy'];
let _groqKeyIndex = 0;
function getGroqKey() { return 'proxy'; }
function rotateGroqKey() { /* no-op: single proxy endpoint */ }
Object.defineProperty(window, 'GROQ_API_KEY', { get: getGroqKey });
...
// Model list: prefer fast/light model to save TPM quota; fall back to heavier one
const GROQ_MODELS = [
  'llama-3.1-8b-instant',      // fastest, lowest token cost
  'llama-3.3-70b-versatile',   // high quality fallback
];
let _modelIndex = 0;
function getGroqModel() { return GROQ_MODELS[_modelIndex % GROQ_MODELS.length]; }
function rotateGroqModel() { _modelIndex = (_modelIndex + 1) % GROQ_MODELS.length; }

async function callGroq(messages, maxTokens = 2000) {
  // Cap tokens to reduce quota usage
  const cappedTokens = Math.min(maxTokens, 1500);
  const maxAttempts = GROQ_API_KEYS.length * GROQ_MODELS.length;

  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    const currentKey   = getGroqKey();
    const currentModel = getGroqModel();
    const body    = JSON.stringify({ model: currentModel, max_tokens: cappedTokens, messages });
    const headers = { 'Content-Type': 'application/json' }; // Auth handled by proxy

    let res;
    try {
      res = await fetch(GROQ_ENDPOINT, { method: 'POST', headers, body });
    } catch (fetchErr) {
      break; // Network/CORS — fall through to XHR
    }
    ...
    if (status === 429) {
      // Rate-limited: rotate key first, then model, with backoff
      rotateGroqKey();
      if (attempt % GROQ_API_KEYS.length === GROQ_API_KEYS.length - 1) rotateGroqModel();
      const wait = 2000 + attempt * 1500;
      console.warn(`[Groq] 429 — rotating key/model, waiting ${wait}ms (attempt ${attempt+1})`);
      await delay(wait);
      continue;
    }
  ...
  // XHR fallback
  const headers2 = { 'Content-Type': 'application/json' }; // Auth handled by proxy
  const body2    = JSON.stringify({ model: getGroqModel(), max_tokens: Math.min(maxTokens, 1500), messages });
  try {
    const result = await new Promise((resolve, reject) => {
      const xhr = new XMLHttpRequest();
      xhr.open('POST', GROQ_ENDPOINT);
      Object.entries(headers2).forEach(([k, v]) => xhr.setRequestHeader(k, v));
      xhr.timeout = 40000;
      xhr.onload = () => {
  ...
  if (location.protocol === 'file:') { showLaunchModal(); throw new Error('FILE_PROTOCOL_BLOCKED'); }
  throw new Error('Proxy failed — is start.py running? Check .env has GROQ_API_KEY.');
}
...
function initApiKey() {
  ...
  if (inp) { inp.value = getGroqKey(); inp.disabled = true; inp.placeholder = 'API keys active ✓'; }
}

function saveApiKey() { updateApiStatus(); }
...
function updateApiStatus() {
  ...
  status.textContent = `Groq Active ✓ (${GROQ_API_KEYS.length} keys)`;
}
