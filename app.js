...
// ══════════════════════════════════════
// ── API Key Rotation: automatically tries next key on rate-limit/auth error ──
const GROQ_API_KEYS = [
  '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',
  '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',
  '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',
];
let _groqKeyIndex = 0;
function getGroqKey() { return GROQ_API_KEYS[_groqKeyIndex % GROQ_API_KEYS.length]; }
function rotateGroqKey() { _groqKeyIndex = (_groqKeyIndex + 1) % GROQ_API_KEYS.length; }
// Keep backward-compat reference used by initApiKey / saveApiKey
...
async function callGroq(messages, maxTokens = 2000) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    const headers = { 'Content-Type': 'application/json', 'Authorization': `Bearer ${currentKey}` };
  ...
  // XHR fallback
  const headers2 = { 'Content-Type': 'application/json', 'Authorization': `Bearer ${getGroqKey()}` };
